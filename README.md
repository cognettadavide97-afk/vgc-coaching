...
16 file di test (146 test in tutto) che girano con `pytest`. `conftest.py` è il file di configurazione condiviso: sostituisce il database MySQL con uno SQLite in memoria e finge le integrazioni esterne (email, Calendar, Discord), così la suite gira ovunque — anche in CI — senza toccare nessun servizio vero.
...
| `test_slots.py` | 10 | la lista pubblica degli slot liberi, la creazione da admin e il vincolo di durata (solo slot da 1 ora) |
...
**Scadenza del refresh token — risolta il 2026-09-04, con una prova ancora in corso.** La schermata di consenso OAuth è stata portata da "Testing" a **"In production"**, e i due refresh token (Gmail e Drive) sono stati rigenerati sotto il nuovo stato. Questo dovrebbe eliminare la scadenza; la conferma la darà solo un token ancora vivo **dopo l'11 settembre 2026** — vedi `STATO_PROGETTO.md`, §9.1 voce 1.

Perché contava: finché la schermata restava in "Testing", il `GMAIL_REFRESH_TOKEN` scadeva dopo **7 giorni a prescindere dall'uso** — non dopo 7 giorni di *inattività*, come si è creduto a lungo. Osservato in produzione il 2026-09-02: il token è scaduto pur essendo esercitato ogni giorno. Le email smettevano di partire silenziosamente.

Due dettagli della pubblicazione, se dovessi rifarla su un altro progetto:
